"""
AKAZE + Sparse BAD + Sinkhorn + Essential Matrix Estimator.

This module provides a combined model that connects the AKAZE-based feature
matching pipeline (AKAZE detection, orientation-aware sparse BAD descriptors,
...
Pipeline:
    1. AKAZE feature detection → score map + orientation map
...
class AKAZESparseBADSinkhornWithEssentialMatrix(nn.Module):
    """
    Combined feature matching and Essential Matrix estimation model using AKAZE.

    Chains AKAZE + Sparse BAD + Sinkhorn with Essential Matrix estimation,
    using the actual detected keypoint positions (in pixel space, normalised
    by the camera intrinsic matrix K) for geometrically accurate computation.

    AKAZE provides per-pixel orientation information which is used to rotate
    BAD pair offsets at each keypoint, making the descriptor rotation-invariant.
    ...
    Args:
        ...
        distance_type: Distance metric for Sinkhorn cost matrix ('l1' or 'l2').
                       Default is 'l2'.
        nms_radius: Radius for pipeline-level NMS on the AKAZE score map.
                    Default is 3.
        ...
        top_k: Number of top-probability entries kept per row and per column
               ...
               Default is 3.
        n_iter: Power-iteration steps for the 9x9 eigenvector solve inside
    ...
    def forward(
        ...
        """
        # ── Step 1: AKAZE detection (scores + orientations) ─────────────
        scores1, orient1 = self.detector(image1)   # (1, 1, H, W) each
