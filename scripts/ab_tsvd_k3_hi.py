"""Complete the local-map (K=3) TSVD sweep at high truncation ratios.

Usage: TORCH_THREADS=4 .venv/bin/python scripts/ab_tsvd_k3_hi.py
"""
...
for seq in SEQ:
    ...
    for ratio in RATIOS:
        ...
        print(f"{seq:6} K=3 tsvd={ratio:<6}: ATE={r['ATE_median']:.4f} "
...
json.dump({"params": params, "ratios": RATIOS, "result": result},
          open(REPO / "eval/results/ab_tsvd_k3_hi.json", "w"), indent=2)
print("wrote eval/results/ab_tsvd_k3_hi.json")
