"""Eval-side seq_opt1 ATE for desk / desk2 / room (torch matcher).

Same parameters and graph as the online graph's DEFAULT_PARAMS, evaluated with
the offline driver ``rtl.eval_seq`` so the online-vs-eval gap can be measured.
...
OUT = REPO / "eval/results/eval_seq_opt1_desk.json"
SEQ = ["desk", "desk2", "room"]
...
for seq in SEQ:
    ...
    print(f"{seq:32} eval_seq_opt1: ATE={r['ATE_median']:.5f} "
