"""A/B Truncated-SVD degeneracy mitigation in the pose-graph optimizer.

Config = (kf_local_map_k, tsvd_ratio). Reuses one match cache per sequence.

Usage: TORCH_THREADS=4 .venv/bin/python scripts/ab_tsvd.py
"""
...
# (kf_local_map_k, tsvd_ratio)
CONFIGS = [(1, 0.0), (3, 0.0), (3, 1e-4), (3, 1e-3), (3, 1e-2)]
SEQ = ["desk", "desk2", "room"]
...
for seq in SEQ:
    ...
    row = {}
    for k, tsvd in CONFIGS:
        p = dict(params)
        p["kf_local_map_k"] = k
        p["tsvd_ratio"] = tsvd
        t = time.perf_counter()
        r = rtl.eval_seq(cache, p, cam, matcher, match_cache)
        key = f"K{k}_tsvd{tsvd}"
        row[key] = {"k": k, "tsvd": tsvd, "ATE_median": r["ATE_median"],
                    "n_loop": r["n_loop"], "n_kf": r["n_kf"]}
        print(f"{seq:6} K={k} tsvd={tsvd:<6}: ATE={r['ATE_median']:.4f} "
...
json.dump({"params": params, "configs": CONFIGS, "result": result},
          open(REPO / "eval/results/ab_tsvd.json", "w"), indent=2)
print("wrote eval/results/ab_tsvd.json")
