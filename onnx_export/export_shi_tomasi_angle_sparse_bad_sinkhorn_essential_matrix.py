#!/usr/bin/env python3
"""
ONNX export script for Shi-Tomasi + Angle + Sparse BAD + Sinkhorn + Essential Matrix.

Exports a combined model that detects keypoints, matches them with Sinkhorn,
and estimates the Essential Matrix from the actual detected keypoint positions
using the weighted 8-point algorithm with Hartley normalisation.
...
Shi-Tomasi keypoints for geometrically accurate Essential Matrix estimation.
...
exporter is therefore disabled by default; use the TorchScript-based exporter
(dynamo=False) for reliable export.

Usage:
    python export_shi_tomasi_angle_sparse_bad_sinkhorn_essential_matrix.py \\
        --output model.onnx \\
...
from pytorch_model.feature_detection.shi_tomasi_angle_sparse_bad_sinkhorn_essential_matrix import (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Export Shi-Tomasi + Angle + Sparse BAD + Sinkhorn + "
    ...
    parser.add_argument(
        "--output", "-o",
        type=str,
        default="shi_tomasi_angle_sparse_bad_sinkhorn_essential_matrix.onnx",
        help=(
    ...
    parser.add_argument(
        ...
        default=30,
        help=(
            "Power-iteration steps for the 9×9 eigenvector solve "
    ...
    parser.add_argument(
        ...
        default=10,
        help=(
            "Power-iteration steps for each 3×3 eigenvector solve inside "
...
def main() -> None:
    ...
    print(
        "Exporting Shi-Tomasi + Angle + Sparse BAD + Sinkhorn + "
    ...
    print(f"  Model type: Shi-Tomasi + Angle + Sparse BAD + Sinkhorn + Essential Matrix")
