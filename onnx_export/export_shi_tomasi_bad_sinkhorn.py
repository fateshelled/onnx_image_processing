...
def main():
    ...
    base_model = ShiTomasiBADSinkhornMatcher(
    ...
    print(f"Exporting Shi-Tomasi + BAD + Sinkhorn {model_type} to ONNX...")
    torch.onnx.export(
        model,
    ...
    if args.with_extraction:
        print(f"  Model type: Shi-Tomasi + BAD + Sinkhorn + Match Extraction")
        ...
        print(f"  Model type: Shi-Tomasi + BAD + Sinkhorn")
        print(f"  Input image1 shape: (B, 1, {args.height}, {args.width})")
        print(f"  Input image2 shape: (B, 1, {args.height}, {args.width})")
